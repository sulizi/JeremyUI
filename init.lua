...
-- Localize talent names for non English clients
aura_env.talentLocal = {
...
aura_env.updateTalents = function()
    ...
    local shortName = function( name )
        local tmp = name or ""
        tmp = gsub( tmp, "\'","")
        tmp = gsub( tmp, "-","")
        return gsub( lower( tmp ), "%s+", "_" )
    end
    
    for _, name in pairs( aura_env.talentLocal ) do
        -- initialize default values
        local short = shortName( name )
        Player.talent[ short ] = LibDBCache:find_talent( nil, 0 )
    end
            ...
            if lNodeInfo.entryIDs then
                for _, EntryID in pairs( lNodeInfo.entryIDs ) do
                    ...
                    if lEntryInfo then
                        ...
                        local spellName = aura_env.talentLocal[spellID] or GetSpellInfo( spellID );
                        
                        if spellName then
                            local short = shortName( spellName )
                            Player.talent[ short ] = LibDBCache:find_talent( spellID, activeRank )
                        end
                    end
                end
            end
...
local mw_spells = {
        ...
        reduces_cd = {
            ["rising_sun_kick"] = function( trigger_state ) 
                ...
                if Player.talent.teachings_of_the_monastery.ok then
                    local remaining = aura_env.getCooldown( 107428 ) -- RSK
                    if remaining > 0 then
                        local targets = 1 -- TODO: Ancient Concordance 
                        local totm = Player.findAura( buff.teachings_of_the_monastery )
                        local totm_stacks = ( totm and totm.stacks or 0 )
    ...
    ["touch_of_death"] = {
        ...
        bonus_da = function()
            ...
            if is_execute then
                ...
                return UnitHealthMax("player") * 0.35
            end
        end,
        ready = function()
            if not IsUsableSpell( 322109 ) then
            ...
            local cd = 90
            local is_execute =  UnitHealth( "target" ) < UnitHealthMax( "player" )
            return is_execute or aura_env.target_ttd > ( 0.35 * cd )
        end,
    },
    ["expel_harm"] = {
        type = "self_heal",
    ...
    ["jadefire_stomp"] = {
        ...
        target_count = function()
            -- this is ww spellID, needs to be updated for mw
            return aura_env.learnedFrontalTargets( 388201 )
        end,
        target_multiplier = function( target_count )
            return min( 5, target_count )
    ...
    ["zen_pulse_heal"] = {
        type = "smart_heal",
        spellID = 124081, -- Missle is 405426
        sp = 1.6632,
        may_crit = true,
        background = true,
        action_multiplier = function()
            ...
            local accumulating_mist = Player.findAura( buff.accumulating_mist )
            if accumulating_mist then
                am = am * ( 1 + accumulating_mist.stacks * accumulating_mist_amp )
            end  
            
            return am
        end,
        target_count = function()
            -- Uses enemy target count
            return aura_env.target_count
        end,
    },
    ["zen_pulse"] = {
    ...
    ["sheiluns_gift"] = {
        ...
        action_multiplier = function()
            ...
            -- Doubles for each mist            
            am = am * ( 1 + GetSpellCount( 399491 ) )
            
            return am
        end,
        target_count = function()
            return min( 3, #aura_env.healer_targets )
        end,
        ready = function()
            return GetSpellCount( 399491 ) > 0 
        end,
        trigger = {
            ["gust_of_mists"] = true,
            ["gust_of_mists-2"] = function() return #aura_env.healer_targets >= 2 end,
            ["gust_of_mists-3"] = function() return #aura_env.healer_targets >= 3 end,           
        },        
    },
    ["invigorating_mist"] = {
        type = "smart_heal",
        ...
        background = true,
        target_count = function()
            return #aura_env.healer_targets
        end,
        target_multiplier = function( player )
            if not aura_env.spells["renewing_mists"]
            or not Player.talent.invigorating_mists 
            or not Player.talent.invigorating_mists.ok
            or not player or not player.unitID
            or not aura_env.findUnitAura( player.unitID, aura_env.spells["renewing_mists"].spellID ) then
                return 0
            end
    ...
    ["vivify"] = {
        -- TODO: Does execute time return 0 during SooM?
        type = "smart_heal",
        spellID = 116670,
        sp = 1.41,
        may_crit = true,
        ...
        action_multiplier = function()
            ...
            am = am * Player.talent.improved_vivify.effectN( 1 ).mod
            
            return am
        end,
        trigger = {
            ["gust_of_mists"] = true,
            ["invigorating_mists"] = true,
        },        
    },
    ["essence_font_hot"] = {
        type = "smart_heal",
        ...
        background = true,
        target_count = function()
            return min( 6, #aura_env.healer_targets )
        end,
    },
    ["essence_font"] = {
        -- TODO: Font of Life CDR effect ??
        type = "smart_heal",
        spellID = 191837,
        sp = 0.464094,
        ticks = 3,
        ...
        action_multiplier = function()
            ...
            if Player.talent.font_of_life.ok then
                am = am * ( 1 + font_of_life_amp )
            end
    ...
    ["soothing_mist"] = {
        ...
        target_count = function()
            ...
            if Player.talent.unison.ok then
                targets = 2
            end
            
            return min( targets, #aura_env.healer_targets )
        end,
        mana = function()
            return aura_env.mana_base_cost( 115175 ) * aura_env.base_execute_time( 115175 )
        end,
    },
    ...
    ["gust_of_mists"] = {
        -- TODO: SooM has a chance to proc Gust of Mists ??
        -- TODO: Targets with essence font benefit twice from Gusts of Mists
        type = "smart_heal",
        spellID = 191894,
        sp = function() return ( Player.mast_bonus - 1 ) end,
        may_crit = true,
        background = true,
    },
    ["jade_serpent_soom"] = {
        ...
        execute_time = function()
            return aura_env.base_execute_time( 115313 ) + aura_env.base_execute_time( 115175 )
        end,
        mana = function()
            return aura_env.mana_base_cost( 115313 ) + aura_env.mana_base_cost( 115175 )
        end,
    },
}
